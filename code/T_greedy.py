...
def partial_T_greedy(beta, G, c0, c1, s0_shots, s0_samples_gpu):
    ...
    compute_f_kernel[b2d](f_gpu, s0_samples_gpu, in1_gpu)
    ...
    while True:
        ...
        compute_coversize_kernel[b2d](f_gpu, s0_samples_gpu, covered_gpu, in1_gpu, coversize_gpu)
        cuda.synchronize()
        ...
        if mxcover == 0:
            ...
            do_cover_kernel[b1d](f_gpu, s0_samples_gpu, covered_gpu, in1[best_s1], best_s1)
            T.append(best_s1)
...
def T_greedy_increment(gpu_id, n, split_ratio, beta):
    ...
    for _ in trange(20):
        ...
        if n0 - 1 == MX_SAMPLE:
            ...
            s0_samples_gpu = cuda.to_device(rnd.sample(range(1 << (n0 - 1)), 1 << MX_SAMPLE))  # fix the color of node n-1
            logger.info('sampled')
        ...
        for _ in trange(20):
            ...
            G = nx.erdos_renyi_graph(n, 0.9)
            c0, c1 = split(G, split_ratio)

            Ts = []
            for i in range(1, MX_SAMPLE + 1):
                logger.info(f's0 shots = 2^{i}')
                T = partial_T_greedy(beta, G, c0, c1, s0_shots=1 << i, s0_samples_gpu=s0_samples_gpu)
                Ts.append(len(T))
